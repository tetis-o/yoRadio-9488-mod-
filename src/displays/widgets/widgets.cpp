...
#include "../../core/display.h"   //  for display mode check
#include "../tools/l10n.h"
#include "../tools/psframebuffer.h"
#include "../fonts/AvignonProMedium42.h"// шрифт секунд
#include "../fonts/AvignonProMedium28.h"// шрифт календаря - месяц
#include "../fonts/BahnschriftSemiCondensed14.h"//шрифт календаря
#include "../fonts/BahnschriftSemiCondensed16.h"//шрифт номера канала

//#include "../fonts/Fnirsi40.h"
#include "../fonts/codecs.h"// иконки битрейта
//#include "../fonts/iconsweather.h"// иконки погоды
//#include <core/timekeeper.h>
#include "../../core/timekeeper.h"

// Константы для координат и размеров VU-метра, иконок битрейта
//constexpr int BR_LEFT      = 284+116; // Координата X для иконок битрейта   
//constexpr int BR_TOP       = 114; // 142 Координата Y для иконок битрейта
constexpr int VU_RECT_X    = 120;// Координата X для левого VU-метра
//constexpr int VU_RECT_X_RIGHT  = VU_RECT_X_LEFT;// Координата X для правого VU-метра
constexpr int VU_RECT_WIDTH    = 22;// Ширина прямоугольника VU-метра
constexpr int VU_RECT_HEIGHT   = 15;// Высота прямоугольника VU-метра
constexpr int VU_RECT_Y_LEFT       = 106;// Координата Y для VU-метров
constexpr int VU_RECT_Y_RIGHT      = VU_RECT_Y_LEFT + VU_RECT_HEIGHT +4;// Координата Y для VU-метров


constexpr int ST_LEFT   = 419;//170;
constexpr int ST_TOP    = 13;//94;

constexpr int CAL_LEFT   = 50;
constexpr int CAL_TOP    = 196;
...
void FillWidget::_draw(){
  ...
  // Рисуем иконки wifi и громкости только если не на экране плейлиста
  if (display.mode() != STATIONS) {
    dsp.drawRGBBitmap(346,320-36, wifi, 45, 32);
    dsp.drawRGBBitmap(250,320-36, volume, 38, 32);
     dsp.drawRGBBitmap(0,320-36, ip, 31, 36);
    //dsp.drawRGBBitmap(278,102, station, 46, 32);
  }
  if(display.mode() == PLAYER) {
dsp.drawFastHLine(0, 187, 480, _fgcolor); // Рисуем рамку для информации о станции/SD карте
dsp.setTextColor(_fgcolor, _bgcolor);
    //dsp.setCursor(ST_LEFT-6, ST_TOP+52);
    ...
    if (config.getMode() == PM_SDCARD) {
      ...
      dsp.drawRGBBitmap(ST_LEFT, ST_TOP, sdcard, 51, 54); // sdcard icon for SD playback
      dsp.fillRect(ST_LEFT-6, ST_TOP+42, 54 , 20,  0x0000);
       dsp.setCursor(ST_LEFT+31- 10*strlen(chbuf)/2, ST_TOP+58);
      dsp.print(chbuf);
    } else {
      ...
      dsp.drawRGBBitmap(ST_LEFT, ST_TOP, station4, 51, 54); // station icon for web streams
      dsp.fillRect(ST_LEFT-6, ST_TOP+42, 54 , 20,  0x0000);
      dsp.setCursor(ST_LEFT+31- 10*strlen(chbuf)/2, ST_TOP+58);
      dsp.print(chbuf);     
    } 
      dsp.drawFastVLine(ST_LEFT-10, ST_TOP-5, 75,  _fgcolor);
      dsp.drawFastHLine(ST_LEFT-10, ST_TOP+69, 65,  _fgcolor); 
      dsp.setFont();
  }
}
...
void VuWidget::_draw(){
  ...
  uint16_t peakColor = 0xE68B;//0xFFE0; // YELLOW
...
void ClockWidget::init(WidgetConfig wconf, uint16_t fgcolor, uint16_t bgcolor){
  ...
  if(_fullclock){
    ...
    _clockheight = _timeheight;// + _space +10 + CHARHEIGHT * _dateheight;//date height
  } else {
...
void ClockWidget::_getTimeBounds() {
  ...
  if(_fullclock){
    ...
    _clockwidth = _timewidth+rightside+10; // ширина виджета часов
...
void ClockWidget::_printClock(bool force){
  ...
  if(force){
    ...
    gfx.setCursor(_left(), _top());
    gfx.print(_timebuffer);                                     //hours & minutes
    if(_fullclock){
      // lines, date & dow
      //if(!config.isScreensaver) dsp.drawRGBBitmap(WEATHER_LEFT, WEATHER_TOP, weather4, WEATHER_W, WEATHER_H);

//*******************    Вывод иконок погоды и календаря  ************************
      if(!config.isScreensaver) {                            
        if(network.timeinfo.tm_min % 1 == 0) {
        
           timekeeper.PrintIcon();}
         if(network.timeinfo.tm_min % 2 == 0) timekeeper.PrintWeate(); 
         
    /*dsp.drawRGBBitmap(CAL_LEFT,CAL_TOP, calendar, 84, 90);//иконка календаря   
    char datebuf[16]; 
    dsp.setTextColor(0xffff);
    dsp.setFont(&BahnschriftSemiCondensed16);
    dsp.setTextSize(1);    
        const char* monthStr = utf8Rus(LANG::mnths[network.timeinfo.tm_mon], true);
    uint16_t monthWidth = strlen(monthStr) * 6; //рассчет примерной ширины
    dsp.setCursor(CAL_LEFT + 60 - monthWidth, CAL_TOP +30);  
    dsp.print(monthStr);//месяц

    //цвет для дня недели/числа: воскресенье — красный, иначе — чёрный
    uint16_t dayColor = (network.timeinfo.tm_wday == 0) ? 0xB000 : 0x0000; // RGB565 red / black
   
    const char* dowStr = utf8Rus(LANG::dowf[network.timeinfo.tm_wday], false);
    uint16_t dowWidth = strlen(dowStr) * 9/2; //рассчет примерной ширины
    dsp. setCursor(CAL_LEFT + 59 - dowWidth, CAL_TOP + 93);
     dsp.setTextColor(dayColor);
    dsp.print(dowStr);//день недели
    
    dsp.setFont(&AvignonProMedium42);
       dsp.setTextSize(1);       
       sprintf(_tmp, "%2d", network.timeinfo.tm_mday);
    dsp.setCursor(CAL_LEFT+60-30*strlen(_tmp)/2, CAL_TOP+76);
    dsp.print(_tmp);//День месяца

    dsp.setFont();*/

    dsp.drawRGBBitmap(CAL_LEFT, CAL_TOP, calendar, 84, 90);
dsp.setFont(&BahnschriftSemiCondensed14);
dsp.setTextSize(1);

//dsp.setCursor(CAL_LEFT+2, CAL_TOP+33);
dsp.setTextColor(0xffff);//цвет месяца
const char* monthStr = utf8Rus(LANG::mnths[network.timeinfo.tm_mon], true);
    uint16_t monthWidth = strlen(monthStr) * 5; //рассчет примерной ширины
    dsp.setCursor(CAL_LEFT + 42 - monthWidth, CAL_TOP +33);  
    dsp.print(monthStr);//месяц
...
//dsp.setCursor(CAL_LEFT+8, CAL_TOP+83);
//dsp.print(utf8Rus(LANG::dow[network.timeinfo.tm_wday], false));
const char* dowStr = utf8Rus(LANG::dow[network.timeinfo.tm_wday], false);
    ...
    dsp.setTextColor(dayColor);
    dsp.setCursor(CAL_LEFT + 41 - dowWidth, CAL_TOP + 83);
    dsp.print(dowStr);//день недели


dsp.setFont(&AvignonProMedium28);
dsp.setTextSize(1);

dsp.setTextColor(dayColor);
sprintf(_tmp, "%2d", network.timeinfo.tm_mday);
dsp.setCursor(CAL_LEFT+42-19*strlen(_tmp)/2, CAL_TOP+67);
...
dsp.setFont();
  ...
  //********************************************************************************* */  
      ...
      if(fullClockOnScreensaver){
        //gfx.drawFastVLine(_linesleft-0, _top()-_timeheight, _timeheight, config.theme.div);//left line
       // gfx.drawFastHLine(_linesleft-0, _top()-(_timeheight)/2-18, CHARWIDTH * _superfont * 2 + _space+7, config.theme.div);//seconds line
        

        /*gfx.setFont(&AvignonProMedium22);
        gfx.setTextSize(1);
        gfx.setCursor(_linesleft+_space-249, _top() + _space-53);
        gfx.setTextColor(config.theme.dow, config.theme.background);
        gfx.print(utf8Rus(LANG::dow[network.timeinfo.tm_wday], false));
        sprintf(_tmp, "%02d.%s.%d", network.timeinfo.tm_mday,LANG::mnths[network.timeinfo.tm_mon], network.timeinfo.tm_year+1900);//dow
        ...
        #else
        gfx.setCursor(_left()+_clockwidth-_datewidth-30, _top() + _space-53);
        #endif
        gfx.setTextColor(config.theme.date, config.theme.background);
        gfx.print(_datebuf);                                  //date
        gfx.setFont();
        #endif*/
      }
    }
  ...
  if(_fullclock || _superfont>0){
    gfx.setFont(&AvignonProMedium42);
    gfx.setTextSize(1);
    ...
    if(!_fullclock){
      ...
      gfx.setCursor(_linesleft+_space, _top());
      gfx.print(";;");                                      //clear seconds area
      gfx.setCursor(_linesleft+_space, _top());
    }
    gfx.setTextColor(config.theme.seconds, config.theme.background);
    sprintf(_tmp, "%02d", network.timeinfo.tm_sec);         //seconds
    
    gfx.print(_tmp);                                        //print seconds
    gfx.setFont();
  }
  gfx.setTextSize(Clock_GFXfontPtr==nullptr?TIME_SIZE:1);
  gfx.setFont(Clock_GFXfontPtr);
  ...
  #endif
  dots=!dots;
  gfx.setCursor(_left()+_dotsleft, _top());             //dots position
  gfx.print(":");                                          //blinking dots
  gfx.setFont();
...
if(config.store.vumeter) {
    ...
    //dsp.drawRect(TFT_FRAMEWDT+396, 178, 30, 26,  0xffff);
    //dsp.drawRect(TFT_FRAMEWDT+396+36, 178, 30, 26,  0xffff);
     dsp.setFont();
    dsp.setTextSize(1);
    //dsp.setTextColor(0xE68B, _bgcolor);
    dsp .setTextColor(_fgcolor, _bgcolor);
    //dsp.setCursor(416,184);
    dsp.setCursor(VU_RECT_X + 9, VU_RECT_Y_LEFT + 4);
    dsp.print("L");
    //dsp.setCursor(416 +36, 184);
    dsp.setCursor(VU_RECT_X + 9, VU_RECT_Y_RIGHT + 4);
    dsp.print("R");
  ...
  //dsp.drawRect(_config.left, _config.top, 66, 58, _fgcolor); //draw bitrate box
  dsp.drawFastVLine(_config.left, _config.top-8, 66, _fgcolor);
  //dsp.setFont();
  ...
  if(_bitrate < 9999)  snprintf(_buf, 6, "%dk", _bitrate);
  else {
    float _br = (float)_bitrate / 1000;
    snprintf(_buf, 6, "%.1f", _br);
  }

  dsp.setCursor(_config.left + 33 - 10*strlen(_buf)/2 , _config.top +21);
  //dsp.setCursor(BR_LEFT + _dimension/2 - _charWidth*strlen(_buf)/2 + 4, BR_TOP -18);
  dsp.print(_buf);
  //dsp.print(" kB");
  ...
  switch(_format){
    case BF_MP3:  dsp.drawRGBBitmap(_config.left+1, _config.top+29, mp3, 64 , 28); break;
    case BF_AAC:  dsp.drawRGBBitmap(_config.left+1, _config.top+29, aac, 64 , 28); break;
    case BF_FLAC: dsp.drawRGBBitmap(_config.left+1, _config.top+29, flac, 64 , 28); break;
    case BF_OGG:  dsp.drawRGBBitmap(_config.left+1, _config.top+29, ogg, 64 , 28); break;
    case BF_OPUS: dsp.drawRGBBitmap(_config.left+1, _config.top+29, opus, 64 , 28); break;
    case BF_VOR:  dsp.drawRGBBitmap(_config.left+1, _config.top+29, vor, 64 , 28); break;
...
void BitrateWidget::_clear() {
 // dsp.fillRect(_config.left, _config.top, _dimension+48, _dimension/2, _bgcolor);
  dsp.fillRect(VU_RECT_X, VU_RECT_Y_LEFT, VU_RECT_WIDTH, VU_RECT_HEIGHT*2+5,  _bgcolor);
  dsp.fillRect(_config.left, _config.top-2, 66, 60, _bgcolor);
}
